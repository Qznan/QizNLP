...
class Run_Model_S2L(Run_Model_Base):
    ...
    def train(self, ckpt_dir, raw_data_file, preprocess_raw_data, batch_size=100):
        ...
        # 字典大小对齐
        assert all([self.model.conf.vocab_size == len(self.token2id_dct['char2id']),
                    self.model.conf.label_size == len(self.token2id_dct['bmeo2id']),
                    ]), f'{self.model.conf.vocab_size} != {len(self.token2id_dct["char2id"])} or {self.model.conf.label_size} != {len(self.token2id_dct["bmeo2id"])}'
